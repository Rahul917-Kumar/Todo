...
function App() {
  ...
  /*function to get data from the firebase as soon page will rerender*/  
    ...
    const updateProgress = async( id, data , inprogress , timestamp  )=>{
        ...
        const newTask  = [...allTask.map((datas)=>{
            if(datas.id===id){
                return (
                    {
                        id:id,
      ...
      >
        ...
        <form>
          <div style={{display:"flex" , justifyContent:"center" , flexDirection:"column"}}>
            ...
            />
            <Button type="submit" variant="outlined" onClick={pushToDo} sx={{marginTop:"10px"}}>
              AddTODO
            </Button>
          </div>
        </form>
        ...
        {allTask.map((data) => {
          return (
            <Todo todo={data.data} inprogress={data.inprogress} id={data.id} updateProgress={updateProgress} deleteTodo = {deleteTodo} timestamp ={data.timestamp} />
          );
        })}
      </div>
    </div>
